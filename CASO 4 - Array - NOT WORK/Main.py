...
def StartConection(user, password, codigo_cidade, index, list_time_process):
    global time_out
    global file_Start
    global path_status
    ...
    list_time_process[index] = round(float(time_current), 7)
    return 0
...
class TesteSete():
    ...
    def Management(self):
        ...
        for teste in self.list_range:
            ...
            list_process = []
            list_time_process = Array("d", range(teste))
            for index in range(teste):
            ...
            print(f" Executando teste com {index} usuários.")
            ...
            list_time_process = []
            itera = 0
            for time_process, id_process in zip(list_time_process, list(range(1, len(list_time_process)+1))):
                list_process.append(id_process)
                list_time_process.append(time_process)
                itera+=1
            df_dict = {
