...
def StartConection(user, password, conn):
    global time_out
    global file_Start
    global path_status
...
class TesteSete():
    ...
    def Management(self):
        ...
        for teste in self.list_range:
            ...
            for espera in range(1,10+1+aux_espera_acumulativa):
                print(f"  * [Info] Iniciando em {espera}.")
                time.sleep(1)
            aux_espera_acumulativa+=2
            ...
            for parent_conn, child_conn in list_vars:
                ...
                list_time_process.append(parent_conn.recv())
                child_conn.close()
                itera+=1
            df_dict = {
